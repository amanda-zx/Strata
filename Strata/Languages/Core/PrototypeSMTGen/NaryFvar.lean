...
/-!
# Spine-aware SMT translation for n-ary free-variable and operator application

This file defines an SMT encoder for a restricted fragment of Core `Expression.Expr` and
proves it correct. The fragment is carved out by the typing judgment `LExpr.HasSimpType`,
whose application nodes are typed through the auxiliary application-spine judgment
`LExpr.AppSpine`: a head (a free variable or a primitive `CoreOp`) applied to a list of
base-typed arguments. The encoder `toSMTTerm` (with its spine helper `appToSMTTerm`)
translates such an expression into an SMT `Term`, encoding an n-ary free-variable
application as a single uninterpreted-function (`UF`) application.

Correctness is stated against the SMT-side type checker `Term.typeCheck` and denotation
`SMTTerm.denote`: the encoder is total on well-typed inputs, its output type-checks at the
SMT encoding of the source type, and its denotation matches the LExpr denotation
`simpDenote` under corresponding interpretations (`UFEnvCorresponds`, `BVarEnvCorresponds`,
`OpInterpConsistent`).

Key definitions: `LExpr.HasSimpType`, `LExpr.AppSpine`, `toSMTTerm`, `Term.typeCheck`,
`SMTTerm.denote`, `FVarCtxWF`, `BVarCtxWF`. Key results: `toSMTTerm_type_correct`,
`toSMTTerm_correct`, and their mutual cores `toSMTTerm_typeChecks` and `toSMTTerm_sound`.
-/
...
inductive LExpr.HasSimpType (Φ : FVarCtx) : List LMonoTy → Expression.Expr → LMonoTy → Prop where
  | const c : MonoTyIsBase c.ty → HasSimpType Φ Δ (.const () c) c.ty
  | bvar i τ : Δ[i]? = some τ → MonoTyIsBase τ → HasSimpType Φ Δ (.bvar () i) τ
  ...
  | app fn arg rty : LExpr.AppSpine Φ Δ (.app () fn arg) [] rty →
    HasSimpType Φ Δ (.app () fn arg) rty
  -- A bare free variable is a nullary application head.
  | fvarNullary f τ rty : LExpr.AppSpine Φ Δ (.fvar () f (some τ)) [] rty →
    HasSimpType Φ Δ (.fvar () f (some τ)) rty
  | ite c t τ e : HasSimpType Φ Δ c (.tcons "bool" []) → HasSimpType Φ Δ t τ →
    HasSimpType Φ Δ e τ → HasSimpType Φ Δ (.ite () c t e) τ
  | eq e1 e2 τ : MonoTyIsBase τ → HasSimpType Φ Δ e1 τ → HasSimpType Φ Δ e2 τ →
    HasSimpType Φ Δ (.eq () e1 e2) (.tcons "bool" [])
  | quant qty body k name : MonoTyIsBase qty → HasSimpType Φ (qty :: Δ) body (.tcons "bool" []) →
    HasSimpType Φ Δ (.quant () k name (some qty) (.const () (.boolConst true)) body) (.tcons "bool" [])

/-- Application-spine judgment. `AppSpine Φ Δ e acc rty` types the head-spine `e`
...
inductive LExpr.AppSpine (Φ : FVarCtx) : List LMonoTy → Expression.Expr → List LMonoTy → LMonoTy → Prop where
  | app fn arg aty acc rty : LExpr.HasSimpType Φ Δ arg aty →
    LExpr.AppSpine Φ Δ fn (aty :: acc) rty →
    LExpr.AppSpine Φ Δ (.app () fn arg) acc rty
  ...
  | fvar f τ acc rty : (f.name, τ) ∈ Φ → collectArrowTy τ = (acc, rty) →
    MonoTyIsBase rty → LExpr.AppSpine Φ Δ (.fvar () f (some τ)) acc rty
  ...
  | op o oty acc rty : CoreOpHasType (CoreOp.ofString o.name) acc rty →
    ...
    LExpr.AppSpine Φ Δ (.op () o (some oty)) acc rty
end
...
def buildAppHead (head : Expression.Expr) (acc : List Term) : Except Format Term :=
  ...
  | .op () o (some _) =>
    ...
    | none => .error f!"Unsupported op: {o.name}"
...
-- be SMT-encoded.
mutual
private theorem HasSimpType_base {Φ : FVarCtx} {Δ : List LMonoTy} {e : Expression.Expr}
    {τ : LMonoTy} (he : LExpr.HasSimpType Φ Δ e τ) : LExpr.MonoTyIsBase τ := by
  match he with
...
private theorem AppSpine_base {Φ : FVarCtx} {Δ : List LMonoTy} {e : Expression.Expr}
    {acc : List LMonoTy} {rty : LMonoTy} (hspine : LExpr.AppSpine Φ Δ e acc rty) :
...
theorem toSMTTerm_typeChecks
    {Φ : FVarCtx} {ufs : UFCtx} (huwf : FVarCtxWF Φ ufs)
  ...
  | .app fn arg rty hspine =>
    ...
    exact appSpine_typeChecks huwf hspine hbwf rfl (by simp [Term.typeCheckArgs]) hτ h_ok'
    ...
    | ok ct =>
      ...
      | ok tt =>
        ...
        | ok et =>
          ...
          simp only [Term.typeCheck]
          have ihc := toSMTTerm_typeChecks (smtTy := .bool) huwf hc hbwf hc_ok
            (by simp [baseTyToTermType])
          have iht := toSMTTerm_typeChecks huwf ht hbwf ht_ok hτ
          have ihe := toSMTTerm_typeChecks huwf hee hbwf he_ok hτ
    ...
    | ok t1 =>
      ...
      | ok t2 =>
        ...
        have key1 : ∀ sty, baseTyToTermType τ' = some sty →
          ...
          fun _ h => toSMTTerm_typeChecks huwf he1 hbwf h1_ok h
        have key2 : ∀ sty, baseTyToTermType τ' = some sty →
            Term.typeCheck ufs bvs t2 = some sty :=
          fun _ h => toSMTTerm_typeChecks huwf he2 hbwf h2_ok h
        simp only [Term.typeCheck]
        cases hbase with
    ...
    | ok bodyTm =>
      ...
      have hbwf' : BVarCtxWF (qty :: Δ)
        ...
        · intro n
          ...
          exact this
      have ihbody := toSMTTerm_typeChecks (smtTy := .bool) huwf hbody hbwf'
...
/-- Spine correctness. If `AppSpine Φ Δ e acc rty` and the already-translated
...
theorem appSpine_typeChecks
    ...
    (hspine : LExpr.AppSpine Φ Δ e acc rty)
    {bvs : TermVarCtx} (hbwf : BVarCtxWF Δ bvs ufs)
  ...
  | .app fn arg aty acc' rty harg hrest =>
    ...
    | ok argt =>
      ...
      have h_argt := toSMTTerm_typeChecks huwf harg hbwf h_arg_ok h_saty
      ...
      exact appSpine_typeChecks huwf hrest hbwf hacc' h_acc_tc' hrty h_ok
...
mutual
theorem HasSimpType_implies_HasTypeA {Φ : FVarCtx} {Δ : List LMonoTy}
    {e : Expression.Expr} {τ : LMonoTy}
    (h : LExpr.HasSimpType Φ Δ e τ) : LExpr.HasTypeA Δ e τ := by
  match h with
...
theorem AppSpine_implies_HasTypeA {Φ : FVarCtx} {Δ : List LMonoTy}
...
theorem toSMTTerm_sound
  ...
  | _, _, .app fn arg rty hspine, hτ, hbase, htA, h_ok, htc =>
    ...
    have h_ok' : appToSMTTerm bvs (.app () fn arg) [] = .ok tm := by
      rw [appToSMTTerm]; rw [toSMTTerm] at h_ok; exact h_ok
    have hres := appToSMTTerm_sound huwf hspine hbwf opInterp hop fvarVal bvarVal ufInterp smtEnv
      hfenv hbenv (show baseTysToTermTypes [] = some [] from rfl)
      ...
      | ok tt_tm =>
        ...
        | ok et =>
          ...
          have htm : tm = Term.app (.core .ite) [ct, tt_tm, et] (Term.typeOf tt_tm) := by
            ...
            exact h_ok.symm
          subst htm
          have htc_c := toSMTTerm_typeChecks huwf hc hbwf hc_ok
            (show baseTyToTermType (.tcons "bool" []) = some .bool from rfl)
          have htc_t := toSMTTerm_typeChecks huwf ht hbwf ht_ok hτ
          have htc_e := toSMTTerm_typeChecks huwf hee hbwf he_ok hτ
          have ihc := toSMTTerm_sound huwf hc hbwf opInterp hop fvarVal bvarVal ufInterp smtEnv
            hfenv hbenv (show baseTyToTermType (.tcons "bool" []) = some .bool from rfl)
            .bool (HasSimpType_implies_HasTypeA hc) hc_ok htc_c
          have iht := toSMTTerm_sound huwf ht hbwf opInterp hop fvarVal bvarVal ufInterp smtEnv
            hfenv hbenv hτ hbase (HasSimpType_implies_HasTypeA ht) ht_ok htc_t
          have ihe := toSMTTerm_sound huwf hee hbwf opInterp hop fvarVal bvarVal ufInterp smtEnv
            hfenv hbenv hτ hbase (HasSimpType_implies_HasTypeA hee) he_ok htc_e
  ...
  | _, _, .eq e1 e2 τ' hb he1 he2, hτ, hbase, htA, h_ok, htc =>
    ...
    | ok t1 =>
      ...
      | ok t2 =>
        ...
        have htc1 := toSMTTerm_typeChecks huwf he1 hbwf h1_ok hτ'_spec
        have htc2 := toSMTTerm_typeChecks huwf he2 hbwf h2_ok hτ'_spec
        have ih1 := toSMTTerm_sound huwf he1 hbwf opInterp hop fvarVal bvarVal ufInterp smtEnv
          hfenv hbenv hτ'_spec hb (HasSimpType_implies_HasTypeA he1) h1_ok htc1
        have ih2 := toSMTTerm_sound huwf he2 hbwf opInterp hop fvarVal bvarVal ufInterp smtEnv
          hfenv hbenv hτ'_spec hb (HasSimpType_implies_HasTypeA he2) h2_ok htc2
  ...
  | _, _, .quant qty qbody qk qname hb hbody, hτ, hbase, htA, h_ok, htc =>
    ...
    | ok bodyTm =>
      ...
      have hbody_tc : Term.typeCheck ufs (v :: bvs) bodyTm = some .bool :=
        toSMTTerm_typeChecks huwf hbody hbwf' hbody_ok rfl
      ...
      have body_eq : ∀ (x : Lambda.TyDenote simpTcInterp simpTyVarVal qty)
        ...
        intro x smtEnv' henv'
        have ih := toSMTTerm_sound huwf hbody hbwf' opInterp hop fvarVal (.cons x bvarVal)
          ufInterp smtEnv' hfenv henv' (show baseTyToTermType (.tcons "bool" []) = some .bool from rfl)
...
/-- **Semantic preservation of `appToSMTTerm` (spine case).**
    ...
    For an application spine `AppSpine Φ Δ e acc rty`, the function
  ...
  | _, _, _, .app fn arg aty acc' rty' harg hrest,
    ...
    | ok argt =>
      ...
      have h_arg_sound : cast (tyDenote_eq_smtTyDenote (HasSimpType_base harg) h_saty)
        ...
        toSMTTerm_sound huwf harg hbwf opInterp hop fvarVal bvarVal
          ufInterp smtEnv hfenv hbenv h_saty (HasSimpType_base harg) htA_arg h_arg_ok h_argt
      -- Instantiate the IH on the sub-spine `hrest` with the extended accumulator.
      have ih := appToSMTTerm_sound huwf hrest hbwf opInterp hop fvarVal bvarVal ufInterp smtEnv
        hfenv hbenv hacc' h_acc_tc' (.cons vArg accArgVals) h_acc_denote' hrty htA_fn h_ok htc
      rw [ih]
...
theorem toSMTTerm_succeeds
  ...
  | .app fn arg rty hspine =>
    ...
    exact appSpine_succeeds huwf hspine hbwf []
  | .fvarNullary f τ_f rty hspine =>
    -- `toSMTTerm (.fvar ..)` is defeq to `appToSMTTerm (.fvar ..) []`.
    exact appSpine_succeeds huwf hspine hbwf []
  | .ite c t _ e_ hc ht hee =>
    have ihc := toSMTTerm_succeeds huwf hc hbwf
    have iht := toSMTTerm_succeeds huwf ht hbwf
    have ihe := toSMTTerm_succeeds huwf hee hbwf
  ...
  | .eq e1 e2 τ' hbase he1 he2 =>
    have ih1 := toSMTTerm_succeeds huwf he1 hbwf
    have ih2 := toSMTTerm_succeeds huwf he2 hbwf
    unfold toSMTTerm; simp only [bind, Except.bind]
    cases h1_ok : toSMTTerm bvs e1 with
  ...
  | .quant qty qbody qk qname hbase hbody =>
    ...
    have hbwf' : BVarCtxWF (qty :: Δ)
      ...
      · intro n
        ...
        simp only [hstr, hstr2] at this
        exact this
    have ihbody := toSMTTerm_succeeds huwf hbody hbwf'
    unfold toSMTTerm
...
theorem appSpine_succeeds
  ...
  | .app fn arg aty acc' rty harg hrest =>
    ...
    have iharg := toSMTTerm_succeeds huwf harg hbwf
    rw [appToSMTTerm]; simp only [bind, Except.bind]
    cases h_arg_ok : toSMTTerm bvs arg with
    | error e => rw [h_arg_ok] at iharg; exact iharg.elim
    | ok argt => exact appSpine_succeeds huwf hrest hbwf (argt :: accTms)
  | .fvar f τ acc' rty hmem hcollect hbase =>
...
theorem toSMTTerm_type_correct
  ...
  have hsucc := toSMTTerm_succeeds huwf he hbwf
  rw [hτ]
  ...
  | ok tm =>
    ...
    have htc := toSMTTerm_typeChecks huwf he hbwf h_ok hτ
...
theorem toSMTTerm_correct
  ...
  | ok tm =>
    ...
    have htc := toSMTTerm_typeChecks huwf he hbwf h_ok hτ
    exact ⟨rfl, htc, toSMTTerm_sound huwf he hbwf opInterp hop fvarVal bvarVal ufInterp smtEnv
      hfenv hbenv hτ (HasSimpType_base he) (HasSimpType_implies_HasTypeA he) h_ok htc⟩
