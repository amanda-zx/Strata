...
private noncomputable def loop_canfail_sim
    ...
    (hwfv : WellFormedSemanticEvalVal ρ₀.eval)
    (hstarT : ReflTransT (StepStmt P (EvalCmd P) extendEval)
      (.stmt (.loop (.det g) none ([] : List (String × P.Expr)) body md) ρ₀) cfg)
  ...
  | .step _ _ _ (@StepStmt.step_loop_exit _ _ _ _ _ _ _ _ _ _ _ _ _ _ _
      hasInvFailure _ _ hff_iff _ _) hrest =>
    have h_no : hasInvFailure = false := empty_inv_no_failure hff_iff
    subst h_no
  ...
  | .step _ _ _ (@StepStmt.step_loop_enter _ _ _ _ _ _ _ _ _ _ _ _ _ _ _
      hasInvFailure hasMeasureFailure hg _ hff_iff hwfb _ hmf_iff _) hrest_seq =>
    have h_no : hasInvFailure = false := empty_inv_no_failure hff_iff
    have h_no_meas : hasMeasureFailure = false := none_meas_no_failure hmf_iff
    let ρ₀' : Env P :=
      {ρ₀ with hasFailure := ρ₀.hasFailure || hasInvFailure || hasMeasureFailure}
    have hρ₀_eq : ρ₀' = ρ₀ := by
      simp [ρ₀', h_no, h_no_meas, Bool.or_false]
    have h_assume : StepKleeneStar P (EvalCmd P)
        (.stmt (.cmd (.assume "guard" g md)) ρ₀) (.terminal ρ₀) :=
      kleene_assume_terminal hg hwfb
    -- New shape: hrest_seq : .seq (.block .none ρ₀'.store (.stmts body ρ₀')) [loop] →* cfg
    match seqT_canfail hrest_seq hf with
    | .inl ⟨cfg_block, h_block, hf_block, _⟩ =>
      -- Failure inside the block (body)
      ...
      -- Kleene path: step_loop_step produces .seq (.block ρ₀.store (.stmt b' ρ₀)) (.loop b')
      -- where b' = .seq (.cmd (.assume "guard" g md)) b. We descend into the block's
      -- inner body via kleene_seq_inner_star ∘ kleene_block_inner_star, evaluating
      -- the assume then sim_body_canfail to land on a failing config.
      let b' : KleeneStmt P (Cmd P) := .seq (.cmd (.assume "guard" g md)) b
      -- Inside the block, the inner is .stmt b' ρ₀.  step_seq from inside (b' = seq assume b)
      -- gives .seq (.stmt assume ρ₀) b. Then evaluate the assume, propagate via step_seq_done
      -- to .stmt b ρ₀, then sim_body_canfail to kcfg.
      have h_inner_run : StepKleeneStar P (EvalCmd P)
          (.stmt b' ρ₀) kcfg := by
        show StepKleeneStar P (EvalCmd P)
          (.stmt (.seq (.cmd (.assume "guard" g md)) b) ρ₀) kcfg
        -- Step 1: .stmt (seq assume b) ρ₀ → .seq (.stmt assume ρ₀) b
        refine .step _ _ _ .step_seq ?_
        -- Step 2: .seq (.stmt assume ρ₀) b →* .seq (.terminal ρ₀) b (via h_assume)
        refine ReflTrans_Transitive _ _ _ _
          (kleene_seq_inner_star _ _ b h_assume) ?_
        -- Step 3: .seq (.terminal ρ₀) b → .stmt b ρ₀
        refine .step _ _ _ .step_seq_done ?_
        -- Step 4: .stmt b ρ₀ →* kcfg
        exact hkstar
      -- Now lift through .block: .block ρ₀.store (.stmt b' ρ₀) →* .block ρ₀.store kcfg.
      have h_block_run : StepKleeneStar P (EvalCmd P)
          (.block ρ₀.store (.stmt b' ρ₀))
          (.block ρ₀.store kcfg) :=
        kleene_block_inner_star ρ₀.store _ _ h_inner_run
      -- Lift through the outer .seq (with .loop b' as the right side):
      ...
      · -- KleeneConfig.getEnv (.seq (.block _ kcfg) _) = kcfg.getEnv
    ...
    | .inr ⟨ρ_block, h_block_term, h_loop_stmts, hlen_total⟩ =>
      -- Block terminates at ρ_block. Decompose to body's terminal + projection.
      have h_noescape_body : ∀ lbl ρ_x,
          ¬ StepStmtStar P (EvalCmd P) extendEval (.stmts body ρ₀') (.exiting lbl ρ_x) :=
        block_exitsCoveredByBlocks_noEscape P (EvalCmd P) extendEval body hcov ρ₀'
      have ⟨ρ_inner, h_inner_term, heq_ρ_block, _⟩ :=
        blockT_reaches_terminal_noExit extendEval h_block_term h_noescape_body
      -- Decompose [loop] ρ_block to extract failing trace from .stmt (.loop ...).
      -- Use the helper lemma `stmtsT_singleton_canfail` to extract a failing trace.
      have ⟨cfg_loop, h_loop_T, hf_loop, hlen_loop_le⟩ :=
        stmtsT_singleton_canfail (s := .loop (.det g) none ([] : List (String × P.Expr)) body md)
          (ρ₀ := ρ_block) h_loop_stmts hf
      -- The recursive call needs h_loop_T.len < hstarT.len.
      -- hstarT is .step ... hrest_seq with len = 1 + hrest_seq.len.
      -- hrest_seq.len > h_block_term.len + h_loop_stmts.len (from hlen_total).
      -- h_loop_T.len ≤ h_loop_stmts.len. So h_loop_T.len < hrest_seq.len < hstarT.len.
      have hlen_loop : h_loop_T.len < hrest_seq.len := by
        omega
      -- Body terminates at ρ_inner; loop continues from ρ_block
      have hterm_body_eq : StepStmtStar P (EvalCmd P) extendEval (.stmts body ρ₀) (.terminal ρ_inner) :=
        hρ₀_eq ▸ reflTransT_to_prop h_inner_term
      have h_sim_body : StepKleeneStar P (EvalCmd P) (.stmt b ρ₀) (.terminal ρ_inner) :=
      ...
      have hwfv_block : WellFormedSemanticEvalVal ρ_block.eval := by
        rw [heq_ρ_block]; exact hρ₀_eq ▸ hwfv
      -- Recursive call: loop from ρ_block reaches failure
      have ⟨kcfg_loop, hkf_loop, hkstar_loop⟩ :=
        loop_canfail_sim extendEval g body md b sim_body_canfail sim_body_terminal
          hcov hnofd_body ρ_block hwfv_block h_loop_T hf_loop
      -- Build Kleene path:
      --   step_loop_step → .seq (.block ρ₀.store (assume;b)) (loop) →
      --   →* .seq (.terminal ρ_block) (loop) → .stmt loop ρ_block →* failing
      have heq_ρ_block_full : ρ_block =
          { ρ_inner with store := projectStore ρ₀.store ρ_inner.store } := by
        have hstore : ρ₀'.store = ρ₀.store := by rw [hρ₀_eq]
        rw [heq_ρ_block, hstore]
...
private noncomputable def loop_canfail_sim_kleene
    ...
    (hwfv : WellFormedSemanticEvalVal ρ₀.eval)
    (hstarT : ReflTransT (StepStmt P (EvalCmd P) extendEval)
      (.stmt (.loop .nondet none ([] : List (String × P.Expr)) body md) ρ₀) cfg)
    (hf : cfg.getEnv.hasFailure = true) :
    ∃ cfg', (KleeneConfig.getEnv cfg').hasFailure = true ∧
  ...
  | .step _ _ _ (@StepStmt.step_loop_nondet_enter _ _ _ _ _ _ _ _ _ _ _ _ _ _
    ...
    | .inl ⟨cfg_block, h_block, hf_block, _⟩ =>
      ...
      have ⟨kcfg, hkf, hkstar⟩ := sim_body_canfail ρ₀ hwfb hwfv
        ⟨inner_body, hf_inner, hρ₀_eq ▸ reflTransT_to_prop h_inner⟩
      -- Kleene path: step_loop_step → .seq (.block ρ₀.store (.stmt b ρ₀)) (.loop b) →* failing
      have h_block_run : StepKleeneStar P (EvalCmd P)
          (.block ρ₀.store (.stmt b ρ₀))
          (.block ρ₀.store kcfg) :=
    ...
    | .inr ⟨ρ_block, h_block_term, h_loop_stmts, hlen_total⟩ =>
      ...
      have ⟨ρ_inner, h_inner_term, heq_ρ_block, _⟩ :=
        blockT_reaches_terminal_noExit extendEval h_block_term h_noescape_body
      have ⟨cfg_loop, h_loop_T, hf_loop, hlen_loop_le⟩ :=
        stmtsT_singleton_canfail (s := .loop .nondet none ([] : List (String × P.Expr)) body md)
          (ρ₀ := ρ_block) h_loop_stmts hf
      ...
      have hwfb_block : WellFormedSemanticEvalBool ρ_block.eval := by
        rw [heq_ρ_block]; exact hρ₀_eq ▸ hwfb
      have ⟨kcfg_loop, hkf_loop, hkstar_loop⟩ :=
        loop_canfail_sim_kleene extendEval body md b sim_body_canfail sim_body_terminal
    ...
    · intro st ns hsz ht ρ₀ hwfb hwfv ⟨cfg, hfcfg, hstar⟩
      ...
      | .loop guard m' inv body md =>
        ...
        | .det g =>
          ...
          exact loop_canfail_sim extendEval g body md b
        ...
        | .nondet =>
          ...
          subst hinv_empty; subst hm_none
          have hsz_body : Block.sizeOf body ≤ n := by simp_all [Stmt.sizeOf]; omega
          exact loop_canfail_sim_kleene extendEval body md b
            (fun ρ₀ hwfb' hwfv' h => ih.2 body b hsz_body hb ρ₀ hwfb' hwfv' h)
            (fun ρ₀ ρ' hwfb' hwfv' h =>
              (simulation extendEval n).2 body b hsz_body hb ρ₀ ρ' hwfb' hwfv' h)
            (stmtToKleene_some_exitsCovered.blockHelper [] body b hb)
...
theorem detToKleene_overapproximates
  ...
  intro prefixIdents st ns ht _ hmem hpd ρ₀ declaredFuncs hswf
