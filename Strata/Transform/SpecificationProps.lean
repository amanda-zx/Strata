...
theorem overapproximates_triple (L₁ L₂ : Lang P)
    ...
    (hswf : ∀ ρ₀ : Env P, Pre ρ₀ → L₁.initEnvWF [newPrefix] declaredFuncs st ρ₀) :
...
theorem OverapproximatesWhen.toAggressivelyWhen (L₁ L₂ : Lang P)
    (T : L₁.StmtT → Option L₂.StmtT) (pre : L₁.StmtT → Prop) (newPrefix : String)
...
theorem overapproximatesWhen_triple (L₁ L₂ : Lang P)
  ...
  have hpd : PrefixDisjoint newPrefix ([newPrefix].erase newPrefix) := by
    intro p hp; simp [List.erase_cons_head] at hp
  have hr := hsem [newPrefix] st s' ht hsource_pre hmem hpd ρ₀ declaredFuncs (hswf ρ₀ hpre)
  exact htriple ρ₀ ρ' hpre hwfb hf₀ (hr.1 ρ' |>.1 hstar)
